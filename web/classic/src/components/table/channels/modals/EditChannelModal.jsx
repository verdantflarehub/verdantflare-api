...
const EditChannelModal = (props) => {
  ...
  const handleInputChange = (name, value) => {
    ...
    if (name === 'type') {
      ...
      if (value === 60) {
        const lockedMapping = JSON.stringify({
          'verdantflare-sd2': 'doubao-seedance-2.0',
        });
        setInputs((current) => ({
          ...current,
          base_url: 'https://wxmaas.clarmic.com',
          models: ['verdantflare-sd2'],
          model_mapping: lockedMapping,
          create_enabled: false,
          poll_enabled: true,
          max_concurrency: 1,
        }));
        formApiRef.current?.setValue('base_url', 'https://wxmaas.clarmic.com');
        formApiRef.current?.setValue('models', ['verdantflare-sd2']);
        formApiRef.current?.setValue('model_mapping', lockedMapping);
        formApiRef.current?.setValue('create_enabled', false);
        formApiRef.current?.setValue('poll_enabled', true);
        formApiRef.current?.setValue('max_concurrency', 1);
      }
    }
    //setAutoBan
  };
    ...
    if (success) {
      ...
      if (data.setting) {
        try {
          ...
          data.create_enabled = parsedSettings.create_enabled === true;
          data.poll_enabled = parsedSettings.poll_enabled !== false;
          data.max_concurrency = Number(parsedSettings.max_concurrency) || 0;
          data.max_task_cost_microunits_cny =
            Number(parsedSettings.max_task_cost_microunits_cny) || 0;
          data.hard_daily_budget_microunits_cny =
            Number(parsedSettings.hard_daily_budget_microunits_cny) || 0;
        } catch (error) {
          ...
          data.create_enabled = false;
          data.poll_enabled = true;
  ...
  const submit = async () => {
    ...
    if (localInputs.type === 60) {
      if (batch) {
        showInfo(t('Wxmaas Seedance 仅支持单密钥渠道'));
        return;
      }
      localInputs.base_url = 'https://wxmaas.clarmic.com';
      localInputs.models = ['verdantflare-sd2'];
      localInputs.model_mapping = JSON.stringify({
        'verdantflare-sd2': 'doubao-seedance-2.0',
      });
      localInputs.max_concurrency = 1;
      const maxConcurrency = Number(localInputs.max_concurrency);
      const maxTaskCost = Number(localInputs.max_task_cost_microunits_cny);
      const dailyBudget = Number(localInputs.hard_daily_budget_microunits_cny);
      if (maxConcurrency !== 1) {
        showInfo(t('最大并发必须为 1'));
        return;
      }
      if (!Number.isSafeInteger(maxTaskCost) || maxTaskCost <= 0) {
        showInfo(t('单任务成本上限必须为正整数'));
        return;
      }
      if (
        !Number.isSafeInteger(dailyBudget) ||
        dailyBudget <= 0 ||
        dailyBudget < maxTaskCost
      ) {
        showInfo(t('每日硬预算必须为正整数且不低于单任务成本上限'));
        return;
      }
    }
    ...
    if (localInputs.type === 60) {
      channelExtraSettings.create_enabled = localInputs.create_enabled === true;
      channelExtraSettings.poll_enabled = localInputs.poll_enabled === true;
      channelExtraSettings.max_concurrency = Number(
        localInputs.max_concurrency,
      );
      channelExtraSettings.max_task_cost_microunits_cny = Number(
        localInputs.max_task_cost_microunits_cny,
      );
      channelExtraSettings.hard_daily_budget_microunits_cny = Number(
        localInputs.hard_daily_budget_microunits_cny,
      );
    }
    ...
    delete localInputs.create_enabled;
    delete localInputs.poll_enabled;
    delete localInputs.max_concurrency;
    delete localInputs.max_task_cost_microunits_cny;
    delete localInputs.hard_daily_budget_microunits_cny;
  ...
  const batchAllowed =
    (!isEdit || isMultiKeyChannel) && inputs.type !== 57 && inputs.type !== 60;
                    ...
                    {inputs.type === 60 && (
                      <>
                        <Banner
                          type='warning'
                          description={t(
                            'Wxmaas Seedance 固定为单密钥、verdantflare-sd2 → doubao-seedance-2.0 与 720P；为避免产生付费视频，不提供创建测试。',
                          )}
                        />
                        <Card className='!rounded-2xl shadow-sm border-0 mt-3'>
                          ...
                          <Text className='text-xs text-gray-600 block mb-4'>
                            {t(
                              '缺失或非法配置会被后端拒绝；金额单位为人民币微单位。',
                            )}
                          </Text>
                          <Row gutter={12}>
                            <Col span={12}>
                              <Form.Switch
                                field='create_enabled'
                                label={t('允许付费创建')}
                                checkedText={t('开')}
                                uncheckedText={t('关')}
                                onChange={(value) =>
                                  handleInputChange('create_enabled', value)
                                }
                                extraText={t('默认关闭')}
                              />
                            </Col>
                            <Col span={12}>
                              <Form.Switch
                                ...
                                uncheckedText={t('关')}
                                onChange={(value) =>
                                  handleInputChange('poll_enabled', value)
                                }
                                extraText={t('与创建开关相互独立')}
                              />
                            </Col>
                          </Row>
                          <Row gutter={12}>
                            ...
                            <Col span={8}>
                              <Form.InputNumber
                                field='max_task_cost_microunits_cny'
                                label={t('单任务成本上限')}
                                min={1}
                                precision={0}
                                onNumberChange={(value) =>
                            ...
                            <Col span={8}>
                              <Form.InputNumber
                                field='hard_daily_budget_microunits_cny'
                                label={t('每日硬预算（UTC）')}
                                min={1}
                                precision={0}
                                onNumberChange={(value) =>
